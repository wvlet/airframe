...
object Resource:
  ...
  /**
    ...
    * @tparam U
    */
  def open[U](referenceClass: Class[?], resourceFileName: String)(body: BufferedInputStream => U): U =
    val u = find(referenceClass, resourceFileName)
    if u.isEmpty then sys.error("Resource %s (in %s) not found".format(resourceFileName, referenceClass.getSimpleName))

    val s = new BufferedInputStream(u.get.openStream())
    try body(s)
    finally s.close

  private def packagePath(referenceClass: Class[?]): String =
  ...
  private def packagePath(packageName: String): String =
    ...
    if packageAsPath.endsWith("/") then packageAsPath else packageAsPath + "/"
  ...
  def find(referenceClass: Class[?], resourceFileName: String): Option[URL] =
  ...
  def find(absoluteResourcePath: String): Option[URL] =
    find(
      "",
      if absoluteResourcePath.startsWith("/") then absoluteResourcePath.substring(1)
  ...
  case class FileInJar(resourceURL: URL, logicalPath: String, isDirectory: Boolean) extends VirtualFile:
    if resourceURL == null then sys.error("resource URL cannot be null: " + logicalPath)

    def url = resourceURL

  private def extractLogicalName(packagePath: String, resourcePath: String): String =
    val p        = if !packagePath.endsWith("/") then packagePath + "/" else packagePath
  ...
  private def collectFileResources(
    ...
    if resourceFilter(file.getPath) then b += SystemFile(file, logicalName)
    if file.isDirectory then
      for childFile <- file.listFiles do
        val childResourceURL =
          resourceURLString + (if resourceURLString.endsWith("/") then "" else "/") + childFile.getName
  ...
  private def listResources(
    ...
    if resourceURL == null then return Seq.empty
    ...
    if protocol == "file" then
      ...
      while entryEnum.hasMoreElements do
        ...
        if logicalName != null && resourceFilter(logicalName) then
          fileList += FileInJar(jarFileURL, logicalName, jarEntry.isDirectory)
    else throw new UnsupportedOperationException("resources other than file or jar are not supported: " + resourceURL)
  ...
  def listResources(
    ...
    val b = Seq.newBuilder[VirtualFile]
    for u <- findResourceURLs(classLoader, packageName) do b ++= listResources(u, packageName, resourceFilter)
    b.result()
  ...
  def findClasses[A](
    ...
    def componentName(path: String): Option[String] =
      ...
      if dot <= 0 then None
      else Some(path.substring(0, dot).replaceAll("/", "."))
    ...
    for vf <- classFileList; cn <- componentName(vf.logicalPath) do
      val className: String = packageName + "." + cn
      for cl <- findClass(className) do
        if !Modifier.isAbstract(cl.getModifiers) && toSearch.isAssignableFrom(cl) then b += cl.asInstanceOf[Class[A]]
    b.result()
