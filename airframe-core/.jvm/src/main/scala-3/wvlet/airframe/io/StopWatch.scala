...
import wvlet.airframe.log.{LogLevel, LogRecord, Logger}
...
trait Timer extends Serializable:
  ...
  import wvlet.airframe.log.LogLevel.*
...
trait TimeReport extends Ordered[TimeReport]:
  ...
  def averageWithoutMinMax =
    if executionCount > 2 then (s.getElapsedTime - min - max) / (_executionCount - 2)
    else average
  ...
  def toHumanReadableFormat(time: Double): String =
    ...
    val unitIndex =
      if digits.isNaN || digits.isInfinity || digits >= -2.0 then 0
      else
  ...
  def report: String =
    ...
    for (k, v) <- subMeasure do lines += indent(1, v.genReportLine)
...
class StopWatch:
  ...
  def getElapsedTime: Double =
    ...
    else elapsedTimeAccumulated / NANO_UNIT
  ...
  /**
    ...
    */
  def stop: Double =
    if state == State.STOPPED then return 0.0
  ...
  /**
    ...
    */
  def resume: Unit =
    if state == State.RUNNING then return
