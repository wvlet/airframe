...
import wvlet.airframe.log.LogLevelScanner.ScannerState
import wvlet.airframe.io.IOUtil.*
import wvlet.airframe.io.{IOUtil, Resource}

import scala.annotation.tailrec
import scala.concurrent.duration.Duration

object LogLevelScanner:
  private val logger = Logger("wvlet.airframe.log.LogLevelScanner")
...
import wvlet.airframe.log.LogLevelScanner.*
...
private[log] class LogLevelScanner extends Guard:
  ...
  def setConfig(config: LogLevelScannerConfig): Unit =
    guard {
      ...
      if prev.logLevelFileCandidates != config.logLevelFileCandidates then lastScannedMillis = None
