...
trait LoggerBase:
  ...
  inline def error(inline message: Any): Unit =
    if self.isEnabled(LogLevel.ERROR) then self.log(LogLevel.ERROR, LoggerMacros.sourcePos(), message)

  inline def warn(inline message: Any): Unit =
    if self.isEnabled(LogLevel.WARN) then self.log(LogLevel.WARN, LoggerMacros.sourcePos(), message)

  inline def info(inline message: Any): Unit =
    if self.isEnabled(LogLevel.INFO) then self.log(LogLevel.INFO, LoggerMacros.sourcePos(), message)

  inline def debug(inline message: Any): Unit =
    if self.isEnabled(LogLevel.DEBUG) then self.log(LogLevel.DEBUG, LoggerMacros.sourcePos(), message)

  inline def trace(inline message: Any): Unit =
    if self.isEnabled(LogLevel.TRACE) then self.log(LogLevel.TRACE, LoggerMacros.sourcePos(), message)

  inline def error(inline message: Any, inline cause: Throwable): Unit =
    if self.isEnabled(LogLevel.ERROR) then self.logWithCause(LogLevel.ERROR, LoggerMacros.sourcePos(), message, cause)

  inline def warn(inline message: Any, inline cause: Throwable): Unit =
    if self.isEnabled(LogLevel.WARN) then self.logWithCause(LogLevel.WARN, LoggerMacros.sourcePos(), message, cause)

  inline def info(inline message: Any, inline cause: Throwable): Unit =
    if self.isEnabled(LogLevel.INFO) then self.logWithCause(LogLevel.INFO, LoggerMacros.sourcePos(), message, cause)

  inline def debug(inline message: Any, inline cause: Throwable): Unit =
    if self.isEnabled(LogLevel.DEBUG) then self.logWithCause(LogLevel.DEBUG, LoggerMacros.sourcePos(), message, cause)

  inline def trace(inline message: Any, inline cause: Throwable): Unit =
    if self.isEnabled(LogLevel.TRACE) then self.logWithCause(LogLevel.TRACE, LoggerMacros.sourcePos(), message, cause)
