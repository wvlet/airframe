/*
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package wvlet.airframe.http

import wvlet.airframe.http.Http.formatInstant
import wvlet.airframe.http.HttpMessage.{EmptyMessage, Message, StringMessage}
import wvlet.airframe.msgpack.spi.MsgPack

import java.time.Instant

case class Response(
    status: HttpStatus = HttpStatus.Ok_200,
    header: HttpMultiMap = HttpMultiMap.empty,
    message: Message = EmptyMessage
) {
  override def toString: String = s"Response(${status},${header})"
  protected def copyWith(newHeader: HttpMultiMap): Response = this.copy(header = newHeader)
  protected def copyWith(newMessage: Message): Response = this.copy(message = newMessage)

  def statusCode: Int = status.code
  def withStatus(newStatus: HttpStatus): Response = this.copy(status = newStatus)

  // Accessors
  def getHeader(key: String): Option[String] = header.get(key)

  def getAllHeader(key: String): Seq[String] = header.getAll(key)

  def allow: Option[String] = header.get(HttpHeader.Allow)

  def accept: Seq[String] = Http.parseAcceptHeader(header.get(HttpHeader.Accept))

  def authorization: Option[String] = header.get(HttpHeader.Authorization)

  def cacheControl: Option[String] = header.get(HttpHeader.CacheControl)

  def contentType: Option[String] = header.get(HttpHeader.ContentType)

  def contentEncoding: Option[String] = header.get(HttpHeader.ContentEncoding)

  def contentLength: Option[Long] = header.get(HttpHeader.ContentLength).map(_.toLong)

  def date: Option[String] = header.get(HttpHeader.Date)

  def expires: Option[String] = header.get(HttpHeader.Expires)

  def host: Option[String] = header.get(HttpHeader.Host)

  def lastModified: Option[String] = header.get(HttpHeader.LastModified)

  def referer: Option[String] = header.get(HttpHeader.Referer)

  def userAgent: Option[String] = header.get(HttpHeader.UserAgent)

  def xForwardedFor: Option[String] = header.get(HttpHeader.xForwardedFor)

  def xForwardedProto: Option[String] = header.get(HttpHeader.xForwardedProto)


  def withHeader(key: String, value: String): Response =
  {
    copyWith(header.set(key, value))
  }

  def withHeader(newHeader: HttpMultiMap): Response =
  {
    copyWith(newHeader)
  }

  def withHeader(f: HttpMultiMap => HttpMultiMap): Response =
  {
    copyWith(f(header))
  }

  def addHeader(key: String, value: String): Response =
  {
    copyWith(header.add(key, value))
  }

  def removeHeader(key: String): Response =
  {
    copyWith(header.remove(key))
  }

  def withContent(content: Message): Response =
  {
    copyWith(content)
  }

  def withContent(content: String): Response =
  {
    copyWith(StringMessage(content))
  }

  def withContent(content: Array[Byte]): Response =
  {
    copyWith(HttpMessage.byteArrayMessage(content))
  }

  def withJson(json: String): Response =
  {
    copyWith(HttpMessage.stringMessage(json)).withContentTypeJson
  }

  def withJson(json: Array[Byte]): Response =
  {
    copyWith(HttpMessage.byteArrayMessage(json)).withContentTypeJson
  }

  def withMsgPack(msgPack: MsgPack): Response =
  {
    copyWith(HttpMessage.byteArrayMessage(msgPack)).withContentTypeMsgPack
  }

  // Content reader
  def contentString: String =
  {
    message.toContentString
  }

  def contentBytes: Array[Byte] =
  {
    message.toContentBytes
  }

  // HTTP header setting utility methods
  def withAccept(acceptType: String): Response = withHeader(HttpHeader.Accept, acceptType)

  def withAcceptMsgPack: Response = withHeader(HttpHeader.Accept, HttpHeader.MediaType.ApplicationMsgPack)

  def withAcceptJson: Response = withHeader(HttpHeader.Accept, HttpHeader.MediaType.ApplicationJson)

  def withAllow(allow: String): Response = withHeader(HttpHeader.Allow, allow)

  def withAuthorization(authorization: String): Response = withHeader(HttpHeader.Authorization, authorization)

  def withCacheControl(cacheControl: String): Response = withHeader(HttpHeader.CacheControl, cacheControl)

  def withContentType(contentType: String): Response = withHeader(HttpHeader.ContentType, contentType)

  def withContentTypeJson: Response = withContentType(HttpHeader.MediaType.ApplicationJson)

  def withContentTypeMsgPack: Response = withContentType(HttpHeader.MediaType.ApplicationMsgPack)

  def withContentLength(length: Long): Response = withHeader(HttpHeader.ContentLength, length.toString)

  def withDate(date: String): Response = withHeader(HttpHeader.Date, date)

  def withDate(date: Instant) = withHeader(HttpHeader.Date, formatInstant(date))

  def withExpires(expires: String): Response = withHeader(HttpHeader.Expires, expires)

  def withHost(host: String): Response = withHeader(HttpHeader.Host, host)

  def noHost: Response = removeHeader(HttpHeader.Host)

  def withLastModified(lastModified: String): Response = withHeader(HttpHeader.LastModified, lastModified)

  def withReferer(referer: String): Response = withHeader(HttpHeader.Referer, referer)

  def withUserAgent(userAgent: String): Response = withHeader(HttpHeader.UserAgent, userAgent)

  def withXForwardedFor(xForwardedFor: String): Response = withHeader(HttpHeader.xForwardedFor, xForwardedFor)

  def withXForwardedProto(xForwardedProto: String): Response = withHeader(HttpHeader.xForwardedProto, xForwardedProto)

  def isContentTypeJson: Boolean =
  {
    contentType.exists(_.startsWith("application/json"))
  }

  def isContentTypeMsgPack: Boolean =
  {
    contentType.exists(x => x == HttpHeader.MediaType.ApplicationMsgPack || x == "application/x-msgpack")
  }

  def acceptsJson: Boolean =
  {
    accept.exists(x => x == HttpHeader.MediaType.ApplicationJson || x.startsWith("application/json"))
  }

  def acceptsMsgPack: Boolean =
  {
    accept.exists(x => x == HttpHeader.MediaType.ApplicationMsgPack || x == "application/x-msgpack")
  }
}
