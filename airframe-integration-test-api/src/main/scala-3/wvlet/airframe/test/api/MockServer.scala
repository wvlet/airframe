...
/**
  * Mock HTTP server API that simulates httpbin.org and jsonplaceholder.typicode.com endpoints for testing HTTP clients
...
class MockServer:
  ...
  private def parseQueryParams(uri: String): Map[String, String] =
    ...
    if queryStart == -1 then Map.empty
    else
  ...
  private def parseJsonOrNull(content: String): Any =
    try
      if content.nonEmpty && content.trim.startsWith("{") then MessageCodec.of[Map[String, Any]].fromJson(content)
      else null
