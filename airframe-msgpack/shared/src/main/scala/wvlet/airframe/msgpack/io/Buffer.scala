...
class ArrayBuffer(a: Array[Byte], baseOffset: Int, size: Int) extends Buffer {
  ...
  def checkCapacity(offset: Int, requestedLength: Int): Unit = {
    if (offset < size) {
  ...
  def writeBytes(offset: Int, v: Array[Byte], vOffset: Int, length: Int): Int = {
    checkCapacity(offset, length)
    Array.copy(v, vOffset, a, baseOffset + offset, length)
    length
  }

  def writeByteAndByte(offset: Int, b: Byte, v: Byte): Int = {
    checkCapacity(offset, 2)
    val pos = baseOffset + offset
    a(pos) = b
    a(pos + 1) = v
    2
  ...
  def writeByteAndShort(offset: Int, b: Byte, v: Short): Int = {
    checkCapacity(offset, 3)
    val pos = baseOffset + offset
    a(pos) = b
    // Use big-endian order
    a(pos + 1) = ((v & 0xFF00) >> 8).toByte
    a(pos + 2) = (v & 0xFF).toByte
    3
  }

  def writeByteAndInt(offset: Int, b: Byte, v: Int): Int = {
    checkCapacity(offset, 5)
    val pos = baseOffset + offset
    a(pos) = b
    // Use big-endian order
    a(pos + 1) = ((v & 0xFF000000) >> 24).toByte
    a(pos + 2) = ((v & 0xFF0000) >> 16).toByte
    a(pos + 3) = ((v & 0xFF00) >> 8).toByte
    a(pos + 4) = (v & 0xFF).toByte
    5
  }

  def writeByteAndLong(offset: Int, b: Byte, v: Long): Int = {
    checkCapacity(offset, 9)
    val pos = baseOffset + offset
    a(pos) = b
    // Use big-endian order
    a(pos + 1) = ((v & 0xFF00000000000000L) >> 56).toByte
    a(pos + 2) = ((v & 0xFF000000000000L) >> 48).toByte
    a(pos + 3) = ((v & 0xFF0000000000L) >> 40).toByte
    a(pos + 4) = ((v & 0xFF00000000L) >> 32).toByte
    a(pos + 5) = ((v & 0xFF000000L) >> 24).toByte
    a(pos + 6) = ((v & 0xFF0000L) >> 16).toByte
    a(pos + 7) = ((v & 0xFF00L) >> 8).toByte
    a(pos + 8) = (v & 0xFFL).toByte
    9
  }
