...
trait MediaEventAttrs extends SharedEventAttrs {
  ...
  /**
    * Script to be run each time the playback rate changes (like when a user switches to a slow motion or fast forward
    * mode)
    */
  ...
  /**
    * Script to be run when the playing position has changed (like when the user fast forwards to a different point in
    * the media)
    */
  ...
  /**
    * Script to be run when the media has paused but is expected to resume (like when the media pauses to buffer more
    * data)
    */
...
trait FormEventAttrs {
  ...
  /**
    * The select event only fires when text inside a text input or textarea is selected. The event is fired after the
...
trait MouseEventAttrs {
  ...
  /**
    * The click event is raised when the user clicks on an element. The click event will occur after the mousedown and
    * mouseup events.
    *
    * MDN
    */
...
/**
  * Attributes applicable only to the input element. This set is broken out because it may be useful to identify the
  * attributes of the input element separately from other groupings. The attributes permitted by the input element are
...
trait InputAttrs extends GlobalAttrs {
  ...
  /**
    ...
    * formaction attribute on a button or input element.
    *
    * MDN
    */
  ...
  /**
    * This attribute indicates whether the value of the control can be automatically completed by the browser. This
    * attribute is ignored if the value of the type attribute is hidden, checkbox, radio, file, or a button type
  ...
  /**
    * This Boolean attribute lets you specify that a form control should have input focus when the page loads, unless
    * the user overrides it, for example by typing in a different control. Only one form element in a document can have
    * the autofocus attribute, which is a Boolean. It cannot be applied if the type attribute is set to hidden (that is,
  ...
  /**
    * When the value of the type attribute is radio or checkbox, the presence of this Boolean attribute indicates that
    * the control is selected by default; otherwise it is ignored.
    *
    * MDN
    */
  ...
  /**
    * The `enctype` attribute provides the encoding type of the form when it is submitted (for forms with a method of
    * "POST").
    */
  ...
  /**
    * The `formaction` attribute provides the URL that will process the input control when the form is submitted and
    * overrides the default `action` attribute of the `form` element. This should be used only with `input` elements of
    * `type` submit or image.
    */
  lazy val formaction = attr("formaction")

  /**
    * The `formenctype` attribute provides the encoding type of the form when it is submitted (for forms with a method
    * of "POST") and overrides the default `enctype` attribute of the `form` element. This should be used only with the
    * `input` elements of `type` "submit" or "image"
    */
  lazy val formenctype = attr("formenctype")

  /**
    * The `formmethod` attribute specifies the HTTP Method the form should use when it is submitted and overrides the
    * default `method` attribute of the `form` element. This should be used only with the `input` elements of `type`
    * "submit" or "image".
    */
  lazy val formmethod = attr("formmethod")

  /**
    * The `formnovalidate` Boolean attribute specifies that the input of the form should not be validated upon submit
    * and overrides the default `novalidate` attribute of the `form`. This should only be used with `input` elements of
    * of `type` "submit".
    */
  lazy val formnovalidate = attr("formnovalidate")

  /**
    * The `formtarget` provides a name or keyword that indicates where to display the response that is received after
    * submitting the form and overrides the `target` attribute of them `form` element. This should only be used with the
    * `input` elements of `type` "submit" or "image"
    */
  ...
  /**
    * The list attribute refers to a <datalist> element that contains the options for an input element the presents a
    * select list.
    */
  lazy val list = attr("list")

  /**
    * The max attribute specifies the maximum value for an <input> element of type number, range, date, datetime,
    * datetime-local, month, time, or week.
    */
  lazy val max = attr("max")

  /**
    * The min attribute specifies the minimum value for an <input> element of type number, range, date, datetime,
    * datetime-local, month, time, or week.
    */
  lazy val min = attr("min")

  /**
    * This Boolean attribute specifies, when present/true, that the user is allowed to enter more than one value for the
    * <input> element for types "email" or "file". It can also be provided to the <select> element to allow selecting
    * more than one option.
    */
  lazy val multiple = attr("multiple").noValue

  /**
    * The minimum allowed length for the input field. This attribute forces the input control to accept no less than the
    * allowed number of characters. It does not produce any feedback to the user; you must write Javascript to make that
    * happen.
    */
  lazy val minlength = attr("minlength")

  /**
    * The maximum allowed length for the input field. This attribute forces the input control to accept no more than the
    * allowed number of characters. It does not produce any feedback to the user; you must write Javascript to make that
    * happen.
    */
  ...
  /**
    * On form elements (input etc.): Name of the element. For example used by the server to identify the fields in form
    * submits.
    *
    * On the meta tag: This attribute defines the name of a document-level metadata. This document-level metadata name
    * is associated with a value, contained by the content attribute.
    *
    * MDN
    */
  ...
  /**
    ...
    * text, search, url, tel, email, and password. Use the `title` attribute to describe the pattern to the user.
    */
  lazy val pattern = attr("pattern")

  /**
    * A hint to the user of what can be entered in the control. The placeholder text must not contain carriage returns
    * or line-feeds. This attribute applies when the value of the type attribute is text, search, tel, url or email;
  ...
  /**
    * This Boolean attribute indicates that the user cannot modify the value of the control. This attribute is ignored
    * if the value of the type attribute is hidden, range, color, checkbox, radio, file, or a button type.
    *
    * MDN
    */
  ...
  /**
    ...
    * type attribute is hidden, image, or a button type (submit, reset, or button). The :optional and :required CSS
  ...
  /**
    ...
    * password, in which case, it is an integer number of characters. Starting in HTML5, this attribute applies only
    * when the type attribute is set to text, search, tel, url, email, or password; otherwise it is ignored. In
    * addition, the size must be greater than zero. If you don't specify a size, a default value of 20 is used.
    *
    * MDN
    */
  ...
  /**
    * The step attribute specifies the numeric intervals for an <input> element that should be considered legal for the
    * input. For example, if step is 2 on a number typed <input> then the legal numbers could be -2, 0, 2, 4, 6 etc. The
    * step attribute should be used in conjunction with the min and max attributes to specify the full range and
    * interval of the legal values. The step attribute is applicable to <input> elements of the following types: number,
    * range, date, datetime, datetime-local, month, time and week.
    */
  lazy val step = attr("step")

  /**
    * A name or keyword indicating where to display the response that is received after submitting the form. In HTML 4,
    * this is the name of, or a keyword for, a frame. In HTML5, it is a name of, or keyword for, a browsing context (for
    * example, tab, window, or inline frame). The following keywords have special meanings:
    *
    *   - _self: Load the response into the same HTML 4 frame (or HTML5 browsing context) as the current one. This value
    *     is the default if the attribute is not specified.
    *   - _blank: Load the response into a new unnamed HTML 4 window or HTML5 browsing context.
    *   - _parent: Load the response into the HTML 4 frameset parent of the current frame or HTML5 parent browsing
    *     context of the current one. If there is no parent, this option behaves the same way as _self.
    *   - _top: HTML 4: Load the response into the full, original window, canceling all other frames. HTML5: Load the
    *     response into the top-level browsing context (that is, the browsing context that is an ancestor of the current
    *     one, and has no parent). If there is no parent, this option behaves the same way as _self.
  ...
  /**
    ...
    * type such as text/html, text/css, and so on. The common use of this attribute is to define the type of style sheet
    * linked and the most common current value is text/css, which indicates a Cascading Style Sheet format. You can use
    * tpe as an alias for this attribute so you don't have to backtick-escape this attribute.
    *
    * MDN
    */
  ...
  /**
    * The initial value of the control. This attribute is optional except when the value of the type attribute is radio
    * or checkbox.
    *
    * MDN
    */
...
trait Attrs
  ...
  /**
    * This is the single required attribute for anchors defining a hypertext source link. It indicates the link target,
    * either a URL or a URL fragment. A URL fragment is a name preceded by a hash mark (#), which specifies an internal
    * target location (an ID) within the current document. URLs are not restricted to Web (HTTP)-based documents. URLs
    * might use any protocol supported by the browser. For example, file, ftp, and mailto work in most user agents.
    *
    * MDN
    */
  ...
  /**
    * This attribute defines the alternative text describing the image. Users will see this displayed if the image URL
    * is wrong, the image is not in one of the supported formats, or until the image is downloaded.
    *
    * MDN
    */
  ...
  /**
    ...
    * space-separated list of the link types values. The most common use of this attribute is to specify a link to an
    * external style sheet: the rel attribute is set to stylesheet, and the href attribute is set to the URL of an
  ...
  /**
    * If the value of the type attribute is image, this attribute specifies a URI for the location of an image to
    * display on the graphical submit button; otherwise it is ignored.
    *
    * MDN
    */
  ...
  /**
    * The attribute describes the role(s) the current element plays in the context of the document. This can be used,
    * for example, by applications and assistive technologies to determine the purpose of an element. This could allow a
    * user to make informed decisions on which actions may be taken on an element and activate the selected action in a
    * device independent way. It could also be used as a mechanism for annotating portions of a document in a domain
    * specific way (e.g., a legal term taxonomy). Although the role attribute may be used to add semantics to an
    * element, authors should use elements with inherent semantics, such as p, rather than layering semantics on
    * semantically neutral elements, such as div role="paragraph".
    *
  ...
  /**
    * This attribute contains a non-negative integer value that indicates for how many columns the cell extends. Its
    * default value is 1; if its value is set to 0, it extends until the end of the <colgroup>, even if implicitly
    * defined, that the cell belongs to. Values higher than 1000 will be considered as incorrect and will be set to the
    * default value (1).
    *
    * MDN
    */
  ...
  /**
    * This attribute contains a non-negative integer value that indicates for how many rows the cell extends. Its
    * default value is 1; if its value is set to 0, it extends until the end of the table section (<thead>, <tbody>,
  ...
  /**
    ...
    * control; the cols attribute must be specified. soft: The browser ensures that all line breaks in the value consist
  ...
  /**
    * ARIA is a set of special accessibility attributes which can be added to any markup, but is especially suited to
    * HTML. The role attribute defines what the general type of object is (such as an article, alert, or slider).
    * Additional ARIA attributes provide other useful properties, such as a description for a form or the current value
    * of a progressbar.
    *
    * MDN
    */
  object aria {
    ...
    /**
      ...
      * change notifications defined by the aria-relevant attribute. See related aria-relevant.
      */
    ...
    /**
      * Indicates what functions can be performed when the dragged object is released on the drop target. This allows
      * assistive technologies to convey the possible drag options available to users, including whether a pop-up menu
      * of choices is provided by the application. Typically, drop effect functions can only be provided once an object
      * has been grabbed for a drag operation as the drop effect functions available are dependent on the object being
      * dragged.
      */
    ...
    /**
      * Indicates that the element and all of its descendants are not visible or perceivable to any user as implemented
      * by the author. See related aria-disabled.
      */
    ...
    /**
      * Identifies the element (or elements) that labels the current element. See related aria-label and
      * aria-describedby.
      */
    ...
    /**
      * Indicates what user agent change notifications (additions, removals, etc.) assistive technologies will receive
  ...
  /**
    ...
    *
    * If this attribute is present, then the style applies only to its parent element. If absent, the style applies to
    * the whole document.
    */
