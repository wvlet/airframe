...
trait SvgAttrs {
  ...
  /**
    * The baseFrequency attribute represent The base frequencies parameter for the noise function of the <feturbulence>
    * primitive. If two <number>s are provided, the first number represents a base frequency in the X direction and the
  ...
  /**
    ...
    * and end time lists". Each individual value can be one of the following: <offset-value>, <syncbase-value>,
    * <event-value>, <repeat-value>, <accessKey-value>, <wallclock-sync-value> or the keyword indefinite.
    *
  ...
  /**
    * The bias attribute shifts the range of the filter. After applying the kernelMatrix of the <feConvolveMatrix>
  ...
  /**
    ...
    * indicate current user coordinates, are permitted on the coordinate values on the <shape>. The value of auto
  ...
  /**
    * The clip-path attribute bind the element is applied to with a given <clipPath> element As a presentation
  ...
  /**
    * The clipPathUnits attribute defines the coordinate system for the contents of the <clipPath> element. the
    * clipPathUnits attribute is not specified, then the effect is as if a value of userSpaceOnUse were specified. Note
    * that values defined as a percentage inside the content of the <clipPath> are not affected by this attribute. It
  ...
  /**
    ...
    * clip-rule attribute basically works as the fill-rule attribute, except that it applies to <clipPath> definitions.
    *
  ...
  /**
    * The color-profile attribute is used to define which color profile a raster image included through the <image>
  ...
  /**
    * The contentScriptType attribute on the <svg> element specifies the default scripting language for the given
    * document fragment. This attribute sets the default scripting language used to process the value strings in event
    * attributes. This language must be used for all instances of script that do not specify their own scripting
    * language. The value content-type specifies a media type, per MIME Part Two: Media Types [RFC2046]. The default
    * value is application/ecmascript
    *
    * Value <content-type>
    *
    * MDN
  ...
  /**
    * This attribute specifies the style sheet language for the given document fragment. The contentStyleType is
    * specified on the <svg> element. By default, if it's not defined, the value is text/css
    *
    * Value <content-type>
    *
    * MDN
  ...
  /**
    * The cursor attribute specifies the mouse cursor displayed when the mouse pointer is over an element.This attribute
    * behave exactly like the css cursor property except that if the browser suport the <cursor> element, it should
    * allow to use it with the <funciri> notation. As a presentation attribute, it also can be used as a property
    * directly inside a CSS stylesheet, see css cursor for further information.
    *
    * Value auto | crosshair | default | pointer | move | e-resize | ne-resize | nw-resize | n-resize | se-resize |
  ...
  /**
    * For the <circle> and the <ellipse> element, this attribute define the x-axis coordinate of the center of the
    * element. If the attribute is not specified, the effect is as if a value of "0" were specified.For the
    * <radialGradient> element, this attribute define the x-axis coordinate of the largest (i.e., outermost) circle for
    * the radial gradient. The gradient will be drawn such that the 100% gradient stop is mapped to the perimeter of
    * this largest (i.e., outermost) circle. If the attribute is not specified, the effect is as if a value of 50% were
    * specified
    *
    * Value <coordinate>
    *
  ...
  /**
    * For the <circle> and the <ellipse> element, this attribute define the y-axis coordinate of the center of the
    * element. If the attribute is not specified, the effect is as if a value of "0" were specified.For the
    * <radialGradient> element, this attribute define the x-axis coordinate of the largest (i.e., outermost) circle for
    * the radial gradient. The gradient will be drawn such that the 100% gradient stop is mapped to the perimeter of
    * this largest (i.e., outermost) circle. If the attribute is not specified, the effect is as if a value of 50% were
  ...
  /**
    * The ‘orient’ attribute indicates how the marker is rotated when it is placed at its position on the markable
    * element.
    *
    * W3C
    */
  ...
  /**
    * The ‘refX’ attribute defines the reference point of the marker which is to be placed exactly at the marker's
    * position on the markable element. It is interpreted as being in the coordinate system of the marker contents,
  ...
  /**
    * The ‘refY’ attribute defines the reference point of the marker which is to be placed exactly at the marker's
    * position on the markable element. It is interpreted as being in the coordinate system of the marker contents,
    * after application of the ‘viewBox’ and ‘preserveAspectRatio’ attributes.
    *
    * W3C
