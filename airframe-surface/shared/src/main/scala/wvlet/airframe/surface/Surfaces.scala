...
case class MethodRef(owner: Class[_],
                     name: String,
                     paramTypes: Seq[Class[_]],
...
object Primitive {
  ...
  sealed abstract class PrimitiveSurface(rawType: Class[_])
...
case class Alias(override val name: String,
                 override val fullName: String,
                 ref: Surface)
    extends GenericSurface(ref.rawType,
                           ref.typeArgs,
                           ref.params,
                           ref.objectFactory) {
...
case class OptionSurface(override val rawType: Class[_],
                         elementSurface: Surface)
    extends GenericSurface(rawType, Seq(elementSurface)) {
  override def isOption: Boolean = true
}
case class JavaEnumSurface(override val rawType: Class[_])
    extends GenericSurface(rawType)

case class EnumSurface(override val rawType: Class[_],
                       stringExtractor: (Class[_], String) => Option[Any])
    extends GenericSurface(rawType)

case class TupleSurface(override val rawType: Class[_],
                        override val typeArgs: Seq[Surface])
  ...
  override def call(obj: Any, x: Any*) = {
    def unsupported =
      throw new UnsupportedOperationException(
        s"Calling method ${name} is not supported: ${this}")
