...
package object airframe {
  ...
  @deprecated(message = "Use design.bind[A].toProvider(...) or in-trait bindLocal{...} instead", since = "19.10.2")
  def bind[A](provider: => A): A = macro bind0Impl[A]
  @deprecated(message = "Use design.bind[A].toProvider(...) or in-trait bindLocal(D1 => A) instead", since = "19.10.2")
