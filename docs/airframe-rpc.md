...
- __grpc-sync__: Create gRPC blocking client (ServiceGrpcSyncClient)
...
Add a following build setting using sbt-airframe plugin:

```scala
airframeHttpClients := Seq("example.api:grpc-sync")
```

With this setting, gRPC blocking client (ServiceGrpcSyncClient.scala) will be genreated. 
