...
//  implicit class LifeCycleSupport[A:ru.TypeTag](val dep:A)(implicit val ev:ru.TypeTag[A]) extends LogSupport {
//    def onInit(f: A => Unit) : A = macro bindOnInit[A]
//    def onDestroy(f: A => Unit) : A = macro bindOnDestroy[A]
//  }
}
