...
object InjectMacros extends LogSupport {
  ...
  def injectImpl[A: c.WeakTypeTag](c: sm.Context)(ev: c.Tree): c.Expr[A] = {
    ...
    c.Expr(
      q"""{
         val c = wvlet.inject.Inject.findContext(this)
         c.get(${ev})
        }
      """
    )
  ...
  (a: c.Tree, d1: c.Tree, d2: c.Tree): c.Expr[A] = {
    import c.universe._
    c.Expr(q"""{ val c = wvlet.inject.Inject.findContext(this); c.getOrElseUpdate($factory(c.get(${d1}), c.get(${d2}))) }""")
  }
  ...
  (a: c.Tree, d1: c.Tree, d2: c.Tree, d3: c.Tree): c.Expr[A] = {
    import c.universe._
    c.Expr(q"""{ val c = wvlet.inject.Inject.findContext(this); c.getOrElseUpdate($factory(c.get(${d1}), c.get(${d2}), c.get(${d3}))) }""")
  }
  ...
  (a: c.Tree, d1: c.Tree, d2: c.Tree, d3: c.Tree, d4: c.Tree): c.Expr[A] = {
    import c.universe._
    c.Expr(
      q"""{ val c = wvlet.inject.Inject.findContext(this); c.getOrElseUpdate($factory(c.get(${d1}), c.get(${d2}), c.get(${d3}), c.get(${d4}))) }""")
  }
  ...
  (a: c.Tree, d1: c.Tree, d2: c.Tree, d3: c.Tree, d4: c.Tree, d5: c.Tree): c.Expr[A] = {
    import c.universe._
    c.Expr(
      q"""{ val c = wvlet.inject.Inject.findContext(this); c.getOrElseUpdate($factory(c.get(${d1}), c.get(${d2}), c.get(${d3}), c.get(${d4}), c.get(${d5}))) }""")
  }
