...
object Surface {
  ...
  val surfaceCache       = new ConcurrentHashMap[FullName, Surface]().asScala
  val methodSurfaceCache = new ConcurrentHashMap[FullName, Seq[MethodSurface]]().asScala

  def of[A]: Surface = macro SurfaceMacros.of[A]
  def methodsOf[A]: Seq[MethodSurface] = macro SurfaceMacros.methodsOf[A]

  /**
    * Find the root surface of a class
    *
    * @param cl
    * @return
    */
  def get(cl: Class[_]): Option[Surface] = surfaceCache.get(cl.getName)
}
